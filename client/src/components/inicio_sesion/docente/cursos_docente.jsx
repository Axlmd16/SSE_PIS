  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // console.log(data);
        const docente_cursos = data.filter(
          (item) => item.docente_id === id_docente
        );
        // console.log(docente_cursos);
        setDocenteCursos(docente_cursos);
        console.log("Cursos del docente:", docenteCursos);
      } catch (error) {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {docenteCursos.map((curso) => (
            ...
            <Link
              to={`/course/detail/${curso.id}/${curso.asignatura_id}`}
              key={curso.id}
            >
              <div className="bg-white rounded-lg overflow-hidden shadow-md">
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-600">
                    ...
                    {curso.periodo_academico_fecha_fin}
                  </p>
                </div>
              </div>
            </Link>
